// use crate::services::users::schema::roles::dsl::roles;
// use crate::services::users::schema::roles::id;
// use crate::services::users::schema::roles::role_name;
// use crate::services::users::schema::users::dsl::users;
use diesel::prelude::*;
use rocket::serde::json::{json, Value};
// use rust_rocket_postgres::schema::users::dsl::users;
use rust_rocket_postgres::{
    models::models::{NewRole, NewUser, Role, User, UserInputUpdateUser, UserInputUser},
    *,
};
extern crate bcrypt;
...
pub fn get_users() -> Value {
    use rust_rocket_postgres::schema::users::dsl::*;
...
pub fn add_role(role_name: &str) -> Value {
    use rust_rocket_postgres::schema::roles;
...
pub fn create_user(user_details: &UserInputUser) -> Value {
    use rust_rocket_postgres::schema::roles::dsl::*;
    use rust_rocket_postgres::schema::users;
    ...
    let mut role: Vec<Role> = roles
        ...
        .or_filter(role_name.eq(&appropriate_filter))
        .limit(1)
...
pub fn update_user(user_details: &UserInputUpdateUser) -> Value {
    ...
    let updated_user_body: NewUser = NewUser {
        ...
        first_name: &user_details
            .first_name
            .clone()
            .unwrap_or(existing_user[0].first_name.clone()),
        middle_name: &user_details
            .middle_name
            .clone()
            .unwrap_or(existing_user[0].middle_name.clone().unwrap()),
        last_name: &user_details
            .last_name
            .clone()
            .unwrap_or(existing_user[0].last_name.clone()),
        email: &user_details
            .email
            .clone()
            .unwrap_or(existing_user[0].email.clone()),
        role_id: &user_details
            .role_id
            .clone()
            .unwrap_or(existing_user[0].role_id.clone()),
    ...
    let updated_user: User =
        diesel::update(users.filter(email.eq(user_details.email.clone().unwrap())))
